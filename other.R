...
# KNN-------------------------------------------------
CVgeneric_Optimized <- function(generic_classifier, training_features, training_labels, K, lossFunction, mapIndices, knn = -1) {
  #for each fold, find the loss
  set.seed(154)
  losses = c()
  folds = createFolds(1:nrow(d), k=K)
  for(k in 1:K) {
    ...
    #run the classifier
    if (k > 0) {
      predicted = generic_classifier(temp_features_train, temp_features_val, temp_labeled_train, knn)
    } else {
      mod_fit = generic_classifier(temp_features_train, temp_labeled_train)
      predicted = predict(mod_fit, newdata=temp_features_val)
    }
...
getCVknn <- function(dataInput, mapIndices) {
  rownames(dataInput) = NULL
  
  training_features = dataInput[1:dim(dataInput)[1]/1e2, 5:12]
  training_labels = as.factor(dataInput$expert_label)
  
  2:10 %>% sapply(function(k) CVgeneric_Optimized(generic_knn, training_features, training_labels, K, loss1, 
                      mapIndices, k))
}
...
test = method1_test[1:dim(method1_test)[1]/1e2, ]
train = method1_train[1:dim(method1_train)[1]/1e2, ]
...
# 4a-----------------------------------------------
pc = prcomp(train[, 5:12])
scores = pc$x
ggplot() + geom_point(aes(x = scores[,1], y = scores[,2], colour = train$expert_label)) + xlab('PC1') + ylab('PC2')
# -------------------------------------------------