...
data1 %>%
  ...
  ggplot(aes(y = value, x = result1)) + geom_boxplot() +
  facet_wrap(~ Features, scales = "free") +
  labs(title = "Features vs. Prediction Results (Method 1)", 
       y = "Correctness of Predictions")

dat_temp = data1 %>% gather(-result1, key = "Features", value = "value")
dat_temp = dat_temp[(dat_temp$Features == "NDAI"),]
head(dat_temp)
grid.table(dat_temp %>% group_by(result1) %>% summarise(NDAImean = mean(value), NDAIvar = var(value)))

NDAI_FN = dat_temp[(dat_temp$result1 == "False Negative"), ]
NDAI_FN
# -------------------------------------------------
# 4c-----------------------------------------------
deg = 2:4
